document.addEventListener("DOMContentLoaded", function() {
    ...
    function squareClick(event) {
        ...
        if (previousMoveWasCheck) {
            if (selectedPiece.dataset.piece === "pawn") {
                isMoveCorrect = pawnMoveChecker(selectedPiece, square, player);
                validMove = isMoveCorrect;
            } else if (isKingMove(selectedPiece, square) || canCaptureAttackingPiece(square)) {
                isMoveCorrect = true;
                validMove = true;
            } else {
                return;
    ...
    function kingMoveChecker(selectedPiece, movedSquare, player) {
        ...
        if (rowDifference <= 1 && columnDifference <= 1) {
            ...
            if (isSquareAttacked(movedSquare, player)) {
                return false;
            }
            return true;
        }
